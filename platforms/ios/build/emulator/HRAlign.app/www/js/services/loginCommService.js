...
mainModule.factory("updatePasswordFromMobile", function ($resource) {
    return $resource((baseURL + '/api/ChangePasswordController/updatePasswordFromMobile.spr'), {}, {
        'save': {
...
mainModule.factory("loginCommService", function ($rootScope, $filter,
    ...
    var changePasswordNow = function () {

        //$scope.$apply();

        if (passwordObj.oldp == "" || passwordObj.newp == "" || passwordObj.confp == "") {
            showAlert("All fields are mandatory\n")
            return;
        }
        if (!(passwordObj.confp == passwordObj.newp)) {
        ...
        $ionicLoading.show();
        loginDetails.password = passwordObj.oldp
        loginDetails.newp = passwordObj.newp;
        loginDetails.username = localStorage.getItem("userName")
        ...
        LoginService.$save(loginDetails, function (data) {
            ...
            UpdatePasswordFromMobile.$save(loginDetails, function (success) {
                showAlert("Change Password!", "Password changed successfully.")
                passwordObj.oldp = ""
                passwordObj.newp = ""
                passwordObj.confp = ""
                shownGroup = null;
                localStorage.setItem("password", loginDetails.newp)
            }, function (success) {
                showAlert(success)
                $ionicLoading.hide();
            });
    ...
    var getRequisitionApproval = function () {
        return;
    ...
    /// password change popup start 
    $rootScope.changePwdPopup = function () {
        var cppt = `
        <form name="cppForm" style="background: rgba(255,255,255,0.7)">
            <div class="list list-inset" style="margin-top: 5%;background-color:white;border: 1px solid #b3b3b3;">
               <div class="form-group row" style=" margin: 20px 0 !important;">
                   <label class="col-xs-8 col-sm-8">
                       <input type="password" name="txtoldp" id="txtoldp" ng-model="passwordObj.oldp"  placeholder='Old Password'>
                   </label>
                   <div class="col-xs-4 col-sm-4">
                       <a  href="" ng-click="opInput();" >
                           <img  ng-if="eyeClosedOP==true" style="margin-right:-16px;width: 30px;   height: 30px;  " src="./img/eyecross.png" />
                           <img  ng-if="eyeClosedOP==false" style="margin-right:-16px;width: 30px;   height: 30px;  " src="./img/eye.png" />
                       </a>
                   </div>
               </div>
               ...
               <div class="form-group row" style=" margin: 20px 0 !important;">
                   <label class="col-xs-8 col-sm-8">
                       <input type="password" name="txtnewp" id="txtnewp" ng-model="passwordObj.newp"  placeholder='New Password'>
                   </label>
                   <div class="col-xs-4 col-sm-4">
                       <a  href="" ng-click="npInput();" >
                           <img  ng-if="eyeClosedNP==true" style="margin-right:-16px;width: 30px;   height: 30px;  " src="./img/eyecross.png" />
                           <img  ng-if="eyeClosedNP==false" style="margin-right:-16px;width: 30px;   height: 30px;  " src="./img/eye.png" />
                       </a>
                   </div>
               </div>
               </div>
               ...
               <div class="form-group row" style=" margin: 20px 0 !important;">
                   <label class="col-xs-8 col-sm-8">
                       <input type="password" name="txtconfp" id="txtconfp" ng-model="passwordObj.confp"  placeholder='Confirm Password'>
                   </label>
                   <div class="col-xs-4 col-sm-4">
                       <a  href="" ng-click="cpInput();" >
                           <img  ng-if="eyeClosedCP==true" style="margin-right:-16px;width: 30px;   height: 30px;  " src="./img/eyecross.png" />
                           <img  ng-if="eyeClosedCP==false" style="margin-right:-16px;width: 30px;   height: 30px;  " src="./img/eye.png" />
                       </a>
                   </div>
               </div>
               </div>
               ...
               <div class="loginButton" style="margin-top: 0%;margin-left: 0%;
                    margin-right: 0%; ">
                   <button class="button button-block button-positive" style="    width: 90% !important;
                   color: #fff;" ng-click="changePasswordNow()">
                       Change Password
                   </button>
               </div>
               </div>
        ...
        `
        var changePwdPopup = $ionicPopup.show({
            template: cppt,
            title: '',
            scope: $rootScope,
            buttons: [
                {
                    ...
                    onTap: function (e) {
                        alert("cpp")
                        navigator.app.exitApp();
                        //changepwd(sessionStorage.getItem("empId"),sessionStorage.getItem("empCode"),"Y")
                    }
                },
                {
                    text: '<b>Exit App</b>',
    ...
    $rootScope.closeModal = function (index) {
        if (index == 1)
            $rootScope.changePwdPopup.hide();
        if (index == 0)
            $rootScope.changePwdPopup.hide();

    };
    ////////// passwrod change popupu over
    ...
    object.userLogin = function (loginDetails, success, error) {
        ...
        LoginService.$save(loginDetails, function (data) {
            ...
            // myServerCookie = data.coo.substring(0, data.coo.indexOf(";"))
            jwtByHRAPI = data.accessToken //+ "dfsdjklsfdkljfsd"


            ed = new Date()
            //alert(document.cookie)ss
            //alert((ed - st )/1000)
            //alert("d1 "+ new Date())
            ...
            if (data.portalModule) {
                // alert("YES")

            } else {
                alert("Oops! some technical issue, Please close the app and re-run the app again.")
                $ionicLoading.hide()
                return
            }
            ...
            if (data.portalModule.length == 0) {                                     // to check if user has at-least one access rights.
                ...
                if (data.userAllowedForMobileApp === undefined) {
                    sessionStorage.setItem('isUserAllowedForMobileApp', "Y");
                }
                else {
                    sessionStorage.setItem('isUserAllowedForMobileApp', data.userAllowedForMobileApp);


                    if (data.userAllowedForMobileApp == "N") {
                        $ionicLoading.hide()
                        alert("User " + loginDetails.username + " not allowed for mobile app, Please contact HR.")
                        //$state.go('loginCtrl');
                        localStorage.setItem('rememberMeFlag', 0);
                        $state.go('loginCtrl');
                        return
                ...
                if (data.alert_header === undefined) {
                    sessionStorage.setItem('alert_header', "HRAlign");
                    alert_header = "HRAlign"
                } else {
                    sessionStorage.setItem('alert_header', data.alert_header);
                ...
                //alert("profile phot "+ sessionStorage.getItem('profilePhoto'))
    ...
    function onLoginData(gender) {
        ...
        var fd = new FormData();
        fd.append("empId", requestObject.empId)
        fd.append("companyId", requestObject.companyId)
        fd.append("locationId", requestObject.locationId)
        fd.append("fromDate", requestObject.fromDate)
        fd.append("toDate", requestObject.toDate)
        var st = new Date()

        $.ajax({
            url: baseURL + '/api/eisCompany/getOnLoginData.spr',
            data: fd,
            processData: false,
            contentType: false,
                ...
                */
                ...
                if (success.attendanceCount[0] == undefined) {
                    ...
                    if (getMyHrapiVersionNumber() >= 35) {
                        ...
                        success.GeoTrackingEnabledForUser = "Y"
                        ...
                        if (getMyHrapiVersionNumber() >= 36) {

                            if (success.GeoTrackingEndTime) {
                                GeoTrackingEndTime = success.GeoTrackingEndTime
                                localStorage.setItem('GeoTrackingEndTime', GeoTrackingEndTime);
                            } else {
                                GeoTrackingEndTime = "18:30"
                                localStorage.setItem('GeoTrackingEndTime', "18:30");
                            }
                        }
                        // alert(GeoTrackingEndTime)
                        ...
                        //GeoTrackingIntervalInMinutes = 5
                        //GeoTrackingEndTime  = "23:40"
                        //localStorage.setItem('GeoTrackingEndTime',"23:40");
        ...
        /*function (data) {
            ...
            $ionicLoading.hide();
            alert("error in getlogindata " + data.code)
            commonService.getErrorMessage(data);
        });*/
        object.getDoNutChartCount = function (dateDetails, success, error) {
            ...
            var fd = new FormData();
            fd.append("fromDate", obj.fromDate)
            fd.append("presentDayFlag", obj.presentDayFlag)
            fd.append("doughnutChartCountOrListFlag", obj.doughnutChartCountOrListFlag)
            fd.append("offset", obj.offset)
            fd.append("maxData", obj.maxData)
            fd.append("searchData", obj.searchData)
