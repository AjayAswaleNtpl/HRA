mainModule.controller('transactionDetailsViewTransEmailAbscondingCtrl', function($scope,$rootScope,$timeout, $state, commonService, getYearListService, getYearListService, viewODApplicationService, viewLeaveApplicationService, detailsService, $ionicPopup, getSetService, $ionicLoading,$filter) {
     ...
     $scope.init=function(){
        ...
        $timeout(function () {
          ...
          // fd.append("message","")
            ...
            $.ajax({
                url: (baseURL + '/transaction/transactionRequisition/viewTransEmailAbsconding.spr'),
                data: fd,
                type: 'POST',
                async:false,
                timeout:10000,
                ...
                success : function(success) {
                    ...
                    if (!(success.clientResponseMsg=="OK")){
                            ...
                            $timeout(function () {
                                ...
                                if(document.getElementById("newEmpCode")){
                                    document.getElementById("newEmpCode").value = $scope.transRequisitionForm.transVO.newEmpCode                                
    ...
    $scope.goSave=function(apprOrRej){
       ...
       // fd.append("getTotalCTC",parseFloat(document.getElementById("payableAmount").value))
        ...
        if(document.getElementById("newEmpCode")){
            transVOToSend.newEmpCode = document.getElementById("newEmpCode").value
        }else{
            transVOToSend.newEmpCode = ""
        }
        ...
        confirmPopup.then(function(res){
            if(res){
            $.ajax({
                url: (baseURL + '/transaction/transactionRequisition/processAbscondingRequisition.spr'),
                data: fd,
                type: 'POST',
                async:false,
                timeout: 120000,
