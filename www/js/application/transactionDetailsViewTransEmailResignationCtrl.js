mainModule.controller('transactionDetailsViewTransEmailResignationCtrl', function($scope,$rootScope,$timeout, $state, commonService, getYearListService, getYearListService, viewODApplicationService, viewLeaveApplicationService, detailsService, $ionicPopup, getSetService, $ionicLoading,$filter) {
    ...
    $scope.init=function(){
        ...
        $timeout(function () {
            ...
            $scope.currTran = tran;
            $scope.currTran.comment = $scope.currTran.comment.replaceAll("<br>","")
          ...
          // fd.append("message","")
            ...
            $.ajax({
                url: (baseURL + '/transaction/transactionRequisition/viewTransEmailResignation.spr'),
                data: fd,
                type: 'POST',
                async:false,
                timeout:10000,
                ...
                success : function(success) {
                    ...
                    if (!(success.clientResponseMsg=="OK")){
                            ...
                            $timeout(function () {
                                ...
                                $scope.isLastLevel = success.isLastLevel
                                $scope.listArtEmps = success.listArtEmps
                                $scope.listFrtEmps = success.listFrtEmps
                                $scope.totalInprocessAppsPending = success.totalInprocessAppsPending
                                $scope.actionApprv = success.actionApprv
                                $scope.LeaveTrackerVO = success.LeaveTrackerVO
                                $scope.TransactionTrackerVO = success.TransactionTrackerVO
                                $scope.ODTrackerVO = success.ODTrackerVO
                                $scope.ClaimTrackerVO = success.ClaimTrackerVO
                                $scope.TravelTrackerVO = success.TravelTrackerVO
                                $scope.MusterTrackerVO = success.MusterTrackerVO
                                $scope.ShiftChangeTrackerVO = success.ShiftChangeTrackerVO
                                $scope.TravelClaimTrackerVO = success.TravelClaimTrackerVO
                                ...
                                $scope.adjustedRecvrDays = success.adjustedRecvrDays
                                //scope.selectedValue.artList = $scope.transRequisitionForm.artCompanyId+""
                                //scope.selectedValue.frtList = $scope.transRequisitionForm.artCompanyId+""
                                $scope.transRequisitionForm.artEmpId+""

                                $scope.initialLwdDate = $scope.toDateStringForm($scope.transRequisitionForm.transVO.initialLwdDate)
                                $scope.wef = $scope.toDateStringForm($scope.transRequisitionForm.transVO.wef)
                                document.getElementById("transTrackerInProcess").value = $scope.TransactionTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessLeave").value = $scope.LeaveTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessOD").value = $scope.ODTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessShift").value = $scope.ShiftChangeTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessTravel").value = $scope.TravelTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessClaim").value = $scope.ClaimTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessMuster").value = $scope.MusterTrackerVO.inProcess
                                document.getElementById("transTrackerInProcessClaimTravel").value = $scope.TravelClaimTrackerVO.inProcess

                                document.getElementById("revlwd").value = $scope.toDateStringForm($scope.transRequisitionForm.transVO.lwd)
                                document.getElementById("recdaysAdj").value = $scope.adjustedRecvrDays
                                $ionicLoading.hide();
    ...
    $scope.goSave=function(appOrRej){
        ...
        var fd = new FormData();
       /* if ('${totalInprocessAppsPending}' != 0) {
            if ('${isLastLevel}' == 'lastLevel') {
                if (!$("input:radio[name='actionApprvType']")
                        .is(":checked")) {
                    warningMessageAlert(
                            "Please Select Action To Be Taken.", '${alertHeader}');
                    return;
                }
                var radioValue = document.form1.actionApprvType.value;
                if (radioValue == 'Assign') {
                    if ('${LeaveTrackerVO.inProcess}' != 0) {
                        var text = onLeaveCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${TransactionTrackerVO.inProcess}' != 0) {
                        var text = onTransactionCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${ODTrackerVO.inProcess}' != 0) {
                        var text = onOdCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${ClaimTrackerVO.inProcess}' != 0) {
                        var text = onClaimCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${TravelTrackerVO.inProcess}' != 0) {
                        var text = onTravelTrackCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${MusterTrackerVO.inProcess}' != 0) {
                        var text = onMusterCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${ShiftChangeTrackerVO.inProcess}' != 0) {
                        var text = onShiftCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                    }
                    if ('${TravelClaimTrackerVO.inProcess}' != 0) {
                        var text = onTravelClaimCust('save');
                        if (text != "") {
                            warningMessageAlert(text, '${alertHeader}');
                            return;
                        }
                ...
                if (radioValue == 'AssignOne') {
                    var text = onAllTrackerCust('save');
                    if (text != "") {
                        warningMessageAlert(text, '${alertHeader}');
                        return;
                    }
        ...
        //transVOtoSend.comments = document.getElementById("comments").value
    ...
    $scope.setContractEndDate = function () {
        ...
        datePicker.show(options, function (date) {
            if (date == undefined) {
                ...
                document.getElementById('contractEndDate').value = $filter('date')(date, 'dd/MM/yyyy');
